...
def create_scheduler(optimizer):
    """
    使用 Jittor 的 LambdaLR 创建带线性预热 + 余弦退火的学习率调度器。
    固定总训练步数为 100 epoch × 650 steps/epoch = 65000，预热步数为 6500。
    每个训练 step 后调用 scheduler.step() 即可自动更新学习率。
    """
    total_steps = 100 * 650      # 总训练迭代步数（可根据实际需要手动修改）
    num_warmup_steps = int(0.1 * total_steps)   # 预热步数，占总步数的 10%

    def lr_lambda(epoch):
        """
        epoch: 当前已调用 step() 的次数（即全局迭代步数）
        """
        if epoch < num_warmup_steps:
            # 预热阶段：线性增长 0 → 1
            return float(epoch) / float(max(1, num_warmup_steps))
        # 余弦退火阶段：1 → 0
        progress = float(epoch - num_warmup_steps) / float(max(1, total_steps - num_warmup_steps))
        return 0.5 * (1.0 + np.cos(np.pi * progress))

    scheduler = jt.optim.LambdaLR(optimizer, lr_lambda=lr_lambda)
    return scheduler
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch in pbar:
                ...
                total_loss += _get_item(loss)
                total_count += 1
                
                current_lr = 1e-5
                if self.scheduler is not None:
                    current_lr = self.optimizer.param_groups[0]['lr']
                pbar.set_description(f"Epoch {epoch}, Loss: {total_loss / total_count}")
                # pbar.set_description(f"Epoch {epoch}, Loss: {_get_item(loss)}")
                self.on_before_optimizer_step(self.optimizer)
                self.optimizer.step()
                if self.scheduler is not None:
                    self.scheduler.step()   # 每个 step 更新学习率
                ...
                if isinstance(validate_dataloader, dict):
                    for name, dataloader in validate_dataloader.items():
                        ...
                        for batch in pbar:
                            ...
                            total_loss += _get_item(loss)
                            total_count += 1
                            pbar.set_description(f"Epoch {epoch}, Validate {name}, Loss: {total_loss / total_count}")
                    ...
                    for batch in pbar:
                        ...
                        total_loss += _get_item(loss)
                        total_count += 1
                        pbar.set_description(f"Epoch {epoch}, Validate, Loss: {total_loss / total_count}")
