...
from .utils import random_euler_rotation, sample_vertex_groups, sample_vertex_groups_with_normal
...
class AugmentAddNoise(Augment):
    ...
    def apply(self, asset: Asset, **kwargs):
        ...
        if asset.scale is not None:
            noise = noise * asset.scale
        # 正常情况下应该是先归一化再添加噪声，但是为了方便在原图形上搜索最近点，将归一化过程调到了最后，
        # 仅仅只是先算好 center 和 scale，所以噪声要乘上 scale，这样最后全部除 scale 时噪声大小就是正确的
...
@dataclass(frozen=True)
class AugmentPatchclosest(Augment):
    ...
    @classmethod
    def parse(cls, **kwargs) -> 'AugmentPatchclosest':
        cls.check_keys(kwargs)
        return AugmentPatchclosest(**kwargs)
    ...
    def apply(self, asset: Asset, **kwargs):
        ...
        # points_flat = pat_t.reshape(-1, 3)
        # mesh = trimesh.Trimesh(vertices=asset.vertices, faces=asset.faces)
        # closest_flat, _, _ = mesh.nearest.on_surface(points_flat)
        # pat_B = closest_flat.reshape(pat_t.shape) # 新的pc_clean:最近点

        points_flat = pat_t.reshape(-1, 3)
        dist, _, closest_flat = igl.point_mesh_squared_distance(points_flat, asset.vertices, asset.faces)
        pat_B = closest_flat.reshape(pat_t.shape)

        # dist1 = (((pat_B - pat_t)**2).sum(axis=1)).mean()
        # print(dist1)
...
@dataclass(frozen=True)
class AugmentNormalizePCpre(Augment):
    
    @classmethod
    def parse(cls, **kwargs) -> 'AugmentNormalizePCpre':
        cls.check_keys(kwargs)
        return AugmentNormalizePCpre(**kwargs)
    ...
    def apply(self, asset: Asset, **kwargs):
        ...
        center = (p_max + p_min) / 2
        pc = pc - center
        asset.scale = np.sqrt((pc**2).sum(axis=1).max()).max()



@dataclass(frozen=True)
class AugmentNormalizePCnoisy(Augment):
    
    @classmethod
    def parse(cls, **kwargs) -> 'AugmentNormalizePCnoisy':
        cls.check_keys(kwargs)
        return AugmentNormalizePCnoisy(**kwargs)
    
    def apply(self, asset: Asset, **kwargs):
        pat_A = asset.meta['pc_noisy']
        pat_B = asset.meta['pc_clean']
        pat_t = asset.meta['pc_mix']

        p_max = pat_A.max(axis=1)
        p_min = pat_A.min(axis=1)
        center = (p_max + p_min) / 2

        pat_A = (pat_A - center) / asset.scale
        pat_B = (pat_B - center) / asset.scale
        pat_t = (pat_t - center) / asset.scale
        asset.meta['pc_noisy'] = pat_A
        asset.meta['pc_clean'] = pat_B
        asset.meta['pc_mix'] = pat_t
        # center使用pcnoisy计算，保证传入的点云居中
        # scale 使用sample的点云计算，保证50000个点的点云归一化
...
def get_augments(*args) -> List[Augment]:
    MAP = {
        ...
        "normalize_pc_noisy": AugmentNormalizePCnoisy,
        "normalize_pc_pre": AugmentNormalizePCpre,
        "add_noise": AugmentAddNoise,
        "linear": AugmentLinear,
        "patch": AugmentPatch,
        "patch_closest": AugmentPatchclosest,
    }
