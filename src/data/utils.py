...
def sample_vertex_groups_with_normal(
    vertices: ndarray,
    faces: ndarray,
    num_samples: int,
    num_vertex_samples: Optional[int]=None,
    ...
    Args:
        ...
        vertex_normals: (N, 3), sampled_normals will be computed if not provided
        
        face_normals: (N, 3), sampled_normals will be computed if not provided
    ...
    # ---------- 确保法向量可用 ----------
    if vertex_normals is None or face_normals is None:
        # 计算面法线（已有向量化）
        v0 = vertices[faces[:, 0]]
        v1 = vertices[faces[:, 1]]
        v2 = vertices[faces[:, 2]]
        ...
        face_normals = face_normals / np.where(norms == 0, 1.0, norms)

        # 计算面积（作为权重）
        areas = 0.5 * np.linalg.norm(np.cross(v1 - v0, v2 - v0), axis=1)

        # 加权法向量
        weighted_normals = face_normals * areas[:, np.newaxis]  # (F,3)
        ...
        vertex_normals = vertex_normals / np.where(norms == 0, 1.0, norms)
    ...
    if vertex_groups is not None:
        ...
        sampled_vertices = np.concatenate([vertices[perm], face_vertices], axis=0)
        # 法向量总是可用的（已确保 vertex_normals 和 face_normals 非 None）
        sampled_normals = np.concatenate([vertex_normals[perm], face_normals[origin_face_index]], axis=0)
